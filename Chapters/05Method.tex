...
\begin{table}[H]
    ...
    \hline
    \end{tabular}
    \caption[Expected relative importance of independent covariates for non-Gaussian GLMMs]{The expected relative importance of the covariates on the latent scale in the different models when they are uncorrelated.}
    \label{table:3}
\end{table}
...
\begin{table}[H]
    ...
    \bottomrule
    \end{tabular}
    \caption[Expected $R^2$ for non-Gaussian GLMMs]{Expected marginal and conditional $R^2$ values on the latent scale for the binomial regression with logit-link (top) and Poisson regression with log-link (bottom) for different correlation levels $\rho$.}
