...
Considering a model with one random intercept, we can no longer compare our model with the LMG method, which is only implemented for the linear regression in the \texttt{relaimpo} R package \citep{gromping_relaimpo}. Therefore, we now compare the BVI method only with the ELMG and ERW methods, which are extensions of the LMG method \citep{matre}.
