...
\end{equation}
where $\mathbf{Z}$ is orthogonal. The contribution from a column of $\mathbf{z_i}$ with respect to a column $\mathbf{x}_j$ is the squared entry $\boldsymbol{\Lambda}_{ij}^2$, with the squared columns of $\mathbf{\Lambda}$ summing to one and acting as weights.
